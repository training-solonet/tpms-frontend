...
const LiveTrackingMapNew = () => {
    ...
    if (map && vehicles) {
      ...
      vehicles.forEach((vehicle) => {
        ...
        const buildIcon = (status) => L.divIcon({
          ...
          `,
          className: 'custom-truck-icon', iconSize: [28, 28], iconAnchor: [14, 28]
        });

        let marker = existing[vehicle.id];
        if (!marker) {
          marker = L.marker(vehicle.position, { icon: buildIcon(vehicle.status), zIndexOffset: 1000, pane: 'markersPane' });
          ...
          try {
            ...
            if (dev) {
              ...
              const latest = statuses.sort((a,b) => new Date(b.reported_at) - new Date(a.reported_at))[0] || null;
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!focus) return;
      const target = vehicles.find(v => String(v.id) === focus)
  ...
  const additionalControls = (
    <>
      ...
      <div className="border-l border-gray-300 pl-3 flex items-center gap-2">
        ...
        <span className="text-xs text-gray-700 font-medium">API {backendOnline ? 'Online' : 'Offline'}</span>
      </div>
      <div className="border-l border-gray-300 pl-3 flex items-center gap-2">
        <div className={`w-2 h-2 rounded-full ${wsStatus === 'connected' ? 'bg-green-500' : wsStatus === 'connecting' || wsStatus === 'reconnecting' ? 'bg-yellow-500' : 'bg-red-500'}`}></div>
