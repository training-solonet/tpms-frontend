...
const LiveTrackingMapNew = () => {
  ...
  const [clusterSelections, setClusterSelections] = useState( // State untuk filter cluster berdasarkan range nomor truk
  ...
  const inSelectedCluster = useCallback(
    (truckId) => {
      ...
      for (const key of clusterSelections) { // Loop setiap range yang dipilih
  ...
  const applyMarkerZoomStyling = useCallback(() => {
    ...
    Object.values(markersRef.current).forEach((marker) => { // Loop semua marker
      try {
  ...
  const loadVehiclesFromBackend = async () => {
    try {
      ...
      if (tpms && tpms.success && Array.isArray(tpms.data)) { // Jika response sukses dan ada data
        items = tpms.data
          .map((d, index) => { // Map setiap item data
            ...
            if (d?.location && Array.isArray(d.location) && d.location.length > 0) { // Jika ada array location
              ...
              lastUpdate = latestLocation?.createdAt // Set waktu update
                ...
                : new Date();
            } else if (d?.location?.lat_lng) { // Fallback ke field lat_lng langsung
            ...
            if (!id || !isValidLat || !isValidLng) { // Jika ID atau koordinat tidak valid
            ...
            return { // Return object kendaraan
      ...
      } else { // Jika TPMS gagal, gunakan legacy backend
        const result = await trucksAPI.getRealTimeLocations(); // Panggil legacy API
        if (!result.success) { // Jika gagal
        ...
        if (data && Array.isArray(data.features)) { // Jika data dalam format GeoJSON features
          items = data.features
            .map((f) => { // Map setiap feature
            ...
            .filter(Boolean); // Filter out nilai null
        } else if (Array.isArray(data)) { // Jika data langsung berupa array
          items = data
            .map((v) => { // Map setiap item
  ...
  const onMapReady = (mapInstance, utils) => {
    ...
    try {
      ...
      if (!markersLayerRef.current) { // Jika layer markers belum dibuat
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => { // Set interval setiap 1 detik
      setVehicles((prevVehicles) => // Update state vehicles
        prevVehicles.map((vehicle) => { // Map setiap kendaraan
          ...
          if (!mapUtils.pointInPolygon(nextPos, mapUtils.polygonLatLng)) { // Jika posisi keluar dari geofence
            ...
            if (!mapUtils.pointInPolygon(nextPos, mapUtils.polygonLatLng)) { // Jika masih keluar
              nextPos = mapUtils.moveByMeters(lastPos, stepM, (baseBearing + 180) % 360); // Putar balik 180 derajat
            }
          }

          setVehicleRoutes((prev) => { // Update rute kendaraan
            ...
            if (moved >= 0.5) { // Jika bergerak minimal 0.5 meter
          ...
          return { // Return object kendaraan yang diupdate
  ...
  useEffect(() => {
    if (map && vehicles) { // Jika map dan vehicles sudah ready
      const L = window.L || require('leaflet'); // eslint-disable-line no-undef
      // Ambil library Leaflet
      if (!markersLayerRef.current) { // Jika layer markers belum dibuat
        try {
      ...
      vehicles.forEach((vehicle) => { // Loop setiap kendaraan
        const colors = { // Palet warna berdasarkan status
        ...
        if (!inSelectedCluster(vehicle.id)) { // Jika kendaraan tidak dalam cluster yang dipilih
          return; // Skip kendaraan ini
        }

        const truckNum = vehicle.truckNumber || extractTruckNumber(vehicle.id) || ''; // Ekstrak nomor truk
        const buildIcon = (status) => // Fungsi untuk build custom icon
          L.divIcon({
            html: ` 
        ...
        if (!marker) { // Jika marker belum ada, buat baru
          ...
          marker.on('click', async () => { // Event handler saat marker diklik
            ...
            try {
              if (liveRouteLineRef.current && map) { // Jika ada rute yang sedang ditampilkan
                try {
              ...
              if (routeHistory.length <= 1) { // Jika rute belum ada/kurang, coba load dari backend
                // Try to load from backend
                try {
                  const histRes = await trucksAPI.getLocationHistory(vehicle.id, { // Request history dari API
                    range: timeRange, // Dengan range waktu yang dipilih
                  });
                  if (histRes.success && Array.isArray(histRes.data)) { // Jika request sukses
                    const coords = histRes.data
                      .map((p) => { // Map setiap point
                      ...
                      .filter(Boolean); // Filter out nilai null
                    if (coords.length > 1) { // Jika ada minimal 2 point
              ...
              if (Array.isArray(routeHistory) && routeHistory.length > 1) { // Jika ada rute dengan minimal 2 point
                const routeColor = '#2563eb'; // Warna biru untuk rute
                liveRouteLineRef.current = L.polyline(routeHistory, { // Buat polyline untuk rute
                ...
                if (liveRouteMarkersRef.current.start) // Jika ada marker start sebelumnya
                  try {
                    map.removeLayer(liveRouteMarkersRef.current.start); // Hapus marker start lama
                  } catch (err) {
                    void err; // Abaikan error
                  }
                if (liveRouteMarkersRef.current.end) // Jika ada marker end sebelumnya
                  try {
                ...
                const startIcon = L.divIcon({ // Icon untuk start point
                ...
                const endIcon = L.divIcon({ // Icon untuk end point
                ...
                liveRouteMarkersRef.current.start = L.marker(routeHistory[0], { // Marker untuk start point
                  icon: startIcon, // Set icon
                  pane: 'routesPane', // Pane untuk rute
                }).addTo(map); // Tambahkan ke peta
                liveRouteMarkersRef.current.end = L.marker(routeHistory[routeHistory.length - 1], { // Marker untuk end point
        ...
        } else { // Jika marker sudah ada sebelumnya
          ...
          if (marker._status !== vehicle.status) { // Jika status berubah
      ...
      Object.keys(existing).forEach((id) => { // Loop semua marker yang ada
        if (!seen.has(id)) { // Jika marker tidak ada di data vehicles terbaru
          try {
            const m = existing[id]; // Ambil marker
            if (m && map.hasLayer(m)) { // Jika marker ada di peta
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (marker) { // Jika marker sudah ada di peta
        try {
      ...
      } else { // Jika marker belum ada (fallback)
  ...
  const additionalControls = ( // JSX untuk kontrol tambahan di header peta
    <>
      ...
      <div className="border-l border-gray-300 pl-3 flex items-center gap-2">
        ...
        ></div>
        <span className="text-xs text-gray-700 font-medium">WS {wsStatus}</span> {/* Label status WebSocket */}
      </div>

      {/* Filter Dropdown */}
      <div className="relative border-l border-gray-300 pl-3"> {/* Container dropdown filter */}
        <button
        ...
        {showFilterDropdown && ( // Tampilkan dropdown jika showFilterDropdown true
          <div className="absolute top-full mt-1 right-0 bg-white border border-gray-200 rounded-lg shadow-lg p-3 min-w-48 z-50"> {/* Panel dropdown */}
            <div className="text-xs font-medium text-gray-700 mb-2">Cluster (Truck No)</div> {/* Header dropdown */}
            <div className="grid grid-cols-1 gap-2 text-xs"> {/* Container checkbox list */}
              {['1-199', '200-399', '400-599', '600-799', '800-999'].map((range) => ( // Loop setiap range cluster
                <label key={range} className="flex items-center gap-2 cursor-pointer select-none"> {/* Label checkbox */}
                  <input
                    type="checkbox" // Checkbox input
                    className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    checked={clusterSelections.has(range)} // Checked jika range ada di Set
                    onChange={(e) => { // Handler saat checkbox diubah
                      setClusterSelections((prev) => { // Update state clusterSelections
                        const next = new Set(prev); // Copy Set lama
                        if (e.target.checked) next.add(range); // Tambahkan range jika checked
            ...
            <div className="mt-2 text-[10px] text-gray-500">Unchecked ranges are hidden</div> {/* Info text */}
          </div>
        )}
      </div>
    </>
  ...
  return ( // Return JSX component
    <>
      ...
      >
        {/* Vehicle Info Card */}
        {showVehicleCard && selectedVehicle && ( // Tampilkan card jika ada kendaraan yang dipilih
          <div
          ...
          >
            ...
            <div className="mb-4 overflow-hidden rounded-lg border border-gray-100"> {/* Container gambar banner */}
              <img src="/icon2.png" alt="Truck" className="h-32 w-full object-cover" /> {/* Gambar truk */}
            </div>
            {/* Header */}
            <div className="flex items-start justify-between"> {/* Container header card */}
              <div> {/* Info kendaraan */}
              ...
              <div className="flex items-center gap-2"> {/* Container status badge dan tombol close */}
                <span
                ...
                >
                  <span
                    className={`inline-block w-1.5 h-1.5 rounded-full ${ // Dot status indicator
                ...
                <button
                  onClick={() => { // Handler saat tombol close diklik
                    setShowVehicleCard(false); // Sembunyikan card
                    setSelectedVehicle(null); // Clear kendaraan yang dipilih
                    if (liveRouteLineRef.current && map) { // Jika ada rute yang ditampilkan
                      try {
                    ...
                    if (liveRouteMarkersRef.current.start) // Jika ada marker start
                      try {
                        map.removeLayer(liveRouteMarkersRef.current.start); // Hapus marker start
                      } catch {
                        /* empty */
                      }
                    if (liveRouteMarkersRef.current.end) // Jika ada marker end
                      try {
            ...
            <div className="mt-4 flex flex-col gap-2"> {/* Container untuk metrik-metrik kendaraan */}
              {/* Speed */}
              <div className="flex items-center justify-between p-3 rounded-lg bg-blue-50/40 border border-blue-100"> {/* Card metrik kecepatan */}
                <div className="flex items-center gap-3">
                  <span className="inline-flex items-center justify-center h-7 w-7 rounded-lg bg-blue-100 border border-blue-200 text-blue-600"> {/* Icon container */}
              ...
              {/* Fuel */}
              <div className="flex items-center justify-between p-3 rounded-lg bg-amber-50/40 border border-amber-100"> {/* Card metrik bahan bakar */}
                <div className="flex items-center gap-3">
                  <span className="inline-flex items-center justify-center h-7 w-7 rounded-lg bg-amber-100 border border-amber-200 text-amber-600"> {/* Icon container */}
                    <span className="material-symbols-outlined text-[18px] leading-none">
                ...
                <div className="text-sm font-semibold text-gray-900">{selectedVehicle.fuel}%</div> {/* Nilai fuel dalam persen */}
              </div>

              {/* Signal */}
              <div className="flex items-center justify-between p-3 rounded-lg bg-violet-50/40 border border-violet-100"> {/* Card metrik signal */}
                <div className="flex items-center gap-3">
                  <span className="inline-flex items-center justify-center h-7 w-7 rounded-lg bg-violet-100 border border-violet-200 text-violet-600"> {/* Icon container */}
                    <span className="material-symbols-outlined text-[18px] leading-none">
                ...
                <div className="text-sm font-semibold text-gray-900">{selectedVehicle.signal}</div> {/* Nilai signal strength */}
              </div>
            </div>

            {/* Last update */}
            <div className="mt-4 flex items-center justify-between rounded-lg border border-gray-200 p-3"> {/* Card waktu update terakhir */}
              <div className="flex items-center gap-2 text-sm text-gray-600">
                <ClockIcon className="w-4 h-4 text-gray-500" /> {/* Icon clock */}
                Last update {/* Label */}
              </div>
              <div className="text-sm font-medium text-gray-900">
                {formatLastUpdate(selectedVehicle.lastUpdate)} {/* Waktu update dalam format relatif (e.g., "5m ago") */}
              </div>
            </div>

            {/* Tire Pressure Display */}
            <div className="mt-5"> {/* Container display tekanan ban */}
              <div className="rounded-lg border border-gray-200 p-3"> {/* Card border */}
            ...
            {/* CTA */}
            <div className="mt-5"> {/* Container tombol CTA */}
              <a
