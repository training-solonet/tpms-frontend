      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-indigo-50 p-6 overflow-y-auto">
        <div className="max-w-3xl mx-auto">
          <div className="mb-6 flex items-center justify-between">
            <h1 className="text-2xl font-semibold text-gray-900">{isNew ? 'Add Vendor' : 'Edit Vendor'}</h1>
            <div className="flex gap-2">
              <button onClick={() => navigate('/vendors')} className="px-3 py-2 rounded-md border text-sm">Back</button>
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-xl shadow p-4 grid grid-cols-1 gap-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Input label="Contact Name" value={form.contact_name} onChange={e => update('contact_name', e.target.value)} />
                <Input label="Contact Phone" value={form.contact_phone} onChange={e => update('contact_phone', e.target.value)} />
                <Input label="Contact Email" value={form.contact_email} onChange={e => update('contact_email', e.target.value)} />
              </div>
            </div>
          )}
        </div>
