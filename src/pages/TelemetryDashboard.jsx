...
const TelemetryDashboard = () => {
  ...
  const getTelemetrySummary = () => {
    ...
    const recentTireEvents = tirePressureEvents.filter(
      (event) => new Date(event.reported_at) > cutoff
    );

    const recentTempEvents = hubTemperatureEvents.filter(
      (event) => new Date(event.reported_at) > cutoff
    );

    // Count alerts
    const pressureAlerts = recentTireEvents.filter(
      (event) => event.exception_type && event.exception_type !== 'normal'
    ).length;

    const tempAlerts = recentTempEvents.filter(
      (event) => event.temperature > 80 // High temperature threshold
    ).length;
    ...
    return {
      ...
      activeTrucks: [
        ...new Set([
          ...recentTireEvents.map((e) => e.truck_id),
          ...recentTempEvents.map((e) => e.device_id),
        ]),
      ].length,
    };
  };
  ...
  const getTruckTelemetry = (truckId) => {
    ...
    // Get latest readings
    const latestPressure = tirePressure.sort(
    ...
    const recentPressureAlerts = tirePressure.filter(
      (event) =>
        new Date(event.reported_at) > last24h &&
        event.exception_type &&
        event.exception_type !== 'normal'
    ).length;

    const recentTempAlerts = hubTemp.filter(
      (event) => new Date(event.reported_at) > last24h && event.temperature > 80
    ).length;
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 mb-8">
            {trucks.map((truck) => {
              ...
              const hasAlerts =
                telemetry.recentPressureAlerts > 0 || telemetry.recentTempAlerts > 0;
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center">
                      <div className={`${hasAlerts ? 'bg-red-100' : 'bg-blue-100'} p-3 rounded-xl`}>
                        <TruckIcon
                          className={`h-6 w-6 ${hasAlerts ? 'text-red-600' : 'text-blue-600'}`}
                        />
                      </div>
                      <div className="ml-4">
                    ...
                    {hasAlerts && <ExclamationTriangleIcon className="h-6 w-6 text-red-500" />}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div className="text-center">
                      <div
                        className={`text-lg font-bold ${hasAlerts ? 'text-red-600' : 'text-green-600'}`}
                      >
                  ...
                  <div className="space-y-2 text-sm">
                    {telemetry.latestPressure && (
                      <div className="flex justify-between">
                        ...
                        <span
                          className={`font-medium text-${getAlertColor(telemetry.latestPressure.exception_type)}-600`}
                        >
                    ...
                    {telemetry.latestTemp && (
                      <div className="flex justify-between">
                        ...
                        <span
                          className={`font-medium text-${getTemperatureColor(telemetry.latestTemp.temperature)}-600`}
                        >
                          ...
                          {getTruckTelemetry(selectedTruck.id).devices.map((device) => (
                            ...
                            >
                              ...
                              <div className="text-sm text-gray-600">
                                ...
                                <p>
                                  Sensors: {sensors.filter((s) => s.device_id === device.id).length}
                                </p>
                              </div>
                            </div>
                          ))}
                  ...
                  {activeTab === 'pressure' && (
                    <div>
                      ...
                      <div className="space-y-3 max-h-96 overflow-y-auto">
                        {getTruckTelemetry(selectedTruck.id)
                          .tirePressure.slice(0, 20)
                          .map((event) => (
                            <div
                              key={event.id}
                              className="bg-gray-50 rounded-xl p-4 border border-gray-200"
                            >
                              <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-2">
                                  <div
                                    className={`w-3 h-3 rounded-full bg-${getAlertColor(event.exception_type)}-500`}
                                  ></div>
                              ...
                              <div className="grid grid-cols-2 gap-4 text-sm text-gray-600">
                                ...
                                <div>
                                  <span>Exception: </span>
                                  <span
                                    className={`font-medium capitalize text-${getAlertColor(event.exception_type)}-600`}
                                  >
                          ...
                          .map((event) => (
                            ...
                            >
                              <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-2">
                                  <div
                                    className={`w-3 h-3 rounded-full bg-${getTemperatureColor(event.temperature)}-500`}
                                  ></div>
                                  <span className="font-medium text-gray-900">
                                    {event.temperature}°C
                                  </span>
