        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
              {clusterStats.map((cs) => (
                <div
                  key={cs.key}
                  className="bg-white/80 backdrop-blur-sm rounded-2xl shadow border border-gray-200 p-4"
                >
                  ...
                  <div className="mt-3 w-full bg-gray-200 rounded-full h-1.5">
                    <div
                      ...
                      style={{
                        width: `${cs.count > 0 ? (cs.active / Math.max(cs.count, 1)) * 100 : 0}%`,
                      }}
                    />
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 mb-8">
            {fleetGroups.map((group) => {
              ...
              const vList = Array.from(vCount.entries())
                .map(([vid, total]) => ({
                  id: vid,
                  name: vid !== 'none' ? byId.get(vid)?.name || 'Unknown' : 'Unassigned',
                  total,
                }))
                .sort((a, b) => (a.id === 'none') - (b.id === 'none'))
                .slice(0, 3);
              return (
                <div
                  key={group.id}
                    ...
                    {loadingTrucks || loadingVendors ? (
                      ...
                      <div className="flex flex-wrap gap-2">
                        {vList.map((v) => (
                          <span
                            key={v.id}
                            className="inline-flex items-center gap-2 px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 text-xs border border-indigo-100"
                          >
                            <span className="font-medium">{v.name}</span>
                            <span className="text-[10px] bg-white/70 rounded px-1 text-gray-600 border border-indigo-100">
                              {v.total}
                            </span>
                          </span>
                        ))}
                        {vCount.size > vList.length && (
                          <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-gray-50 text-gray-600 text-xs border">
                            +{vCount.size - vList.length} more
                          </span>
                        )}
                      </div>
                    )}
  ...
  const items = React.useMemo(() => {
    ...
    for (const [vid, stats] of vendorMap.entries()) {
      ...
      list.push({
        ...
        total: stats.total,
        active: stats.active,
        inactive: Math.max(0, stats.total - stats.active),
      });
    }
