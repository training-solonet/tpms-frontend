...
const DeviceStatus = () => {
      ...
      const deviceData = devices.map((device, idx) => {
        ...
        return {
          ...
          warrantyExpiry: new Date(
            new Date(device.created_at).getTime() + 2 * 365 * 24 * 3600000
          ).toISOString(),
  ...
  const filteredDevices = deviceInfos.filter((device) => {
    ...
    if (filterStatus === 'online')
      return device.status === 'active' && device.connectionStatus === 'connected';
    if (filterStatus === 'offline')
      return device.status === 'inactive' || device.connectionStatus === 'disconnected';
    if (filterStatus === 'warning')
      return device.maintenanceNeeded || device.batteryLevel < 30 || device.alertCount > 0;
    return true;
  });
              ...
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    {
                      deviceInfos.filter(
                        (d) => d.healthStatus === 'warning' || d.healthStatus === 'critical'
                      ).length
                    }
                  </p>
                </div>
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center">
                    <div className={`bg-${getStatusColor(device.healthStatus)}-100 p-3 rounded-xl`}>
                      <CpuChipIcon
                        className={`h-6 w-6 text-${getStatusColor(device.healthStatus)}-600`}
                      />
                    </div>
                    <div className="ml-4">
                      <h3 className="text-lg font-semibold text-gray-900">{device.sn}</h3>
                      <p className="text-sm text-gray-600">
                        {device.truck?.name || 'Unknown Truck'}
                      </p>
                    </div>
                  </div>
                ...
                {device.status && (
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span
                        className={`font-medium ${device.status.lock_state ? 'text-red-600' : 'text-green-600'}`}
                      >
                ...
                <div className="p-6">
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      ...
                      <p className="text-gray-600">
                        {selectedDevice.truck?.name} - {selectedDevice.truck?.plate_number}
                      </p>
                    </div>
                    <button
                  ...
                  <div className="mb-6">
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">
                      Sensors ({selectedDevice.sensors.length})
                    </h3>
                        ...
                        >
                          ...
                          <div className="space-y-1 text-sm text-gray-600">
                            ...
                            <div className="flex justify-between">
                              <span>Status:</span>
                              <span
                                className={`font-medium ${sensor.removed_at ? 'text-red-600' : 'text-green-600'}`}
                              >
                  ...
                  {selectedDevice.status && (
                    <div className="bg-gray-50 rounded-xl p-4">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          ...
                          <div className="font-medium text-lg">
                            {selectedDevice.status.host_bat}%
                          </div>
                        </div>
                        <div>
                          <span className="text-gray-600">Repeater 1:</span>
                          <div className="font-medium text-lg">
                            {selectedDevice.status.repeater1_bat}%
                          </div>
                        </div>
                        <div>
                          <span className="text-gray-600">Repeater 2:</span>
                          <div className="font-medium text-lg">
                            {selectedDevice.status.repeater2_bat}%
                          </div>
                        </div>
                        <div>
                          <span className="text-gray-600">Lock State:</span>
                          <div
                            className={`font-medium text-lg ${selectedDevice.status.lock_state ? 'text-red-600' : 'text-green-600'}`}
                          >
                      ...
                      <div className="mt-4 text-xs text-gray-500">
                        Last reported:{' '}
                        {new Date(selectedDevice.status.reported_at).toLocaleString()}
                      </div>
                    </div>
                  )}
                </div>
