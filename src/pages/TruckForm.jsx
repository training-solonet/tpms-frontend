...
export default function TruckForm() {
  ...
  const clusters = React.useMemo(() => Array.from(new Set(allDummyTrucks.map(t => t.cluster))), []);
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-indigo-50 p-6 overflow-y-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            <Link to="/trucks" className="px-3 py-2 rounded-md border text-sm">Back</Link>
            <button onClick={handleSave} className="px-4 py-2 rounded-md bg-indigo-600 text-white text-sm hover:bg-indigo-700">Save</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <section className="bg-white rounded-xl shadow p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Input label="Plate" value={truck.plate} onChange={e => updateTruck(['plate'], e.target.value)} />
                <Input label="Name" value={truck.name} onChange={e => updateTruck(['name'], e.target.value)} />
                <Select label="Cluster" value={truck.cluster} onChange={e => updateTruck(['cluster'], e.target.value)}>
                  {clusters.map(c => (
            ...
            <section className="bg-white rounded-xl shadow p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Input label="Driver Name" value={truck.driver.name} onChange={e => updateTruck(['driver','name'], e.target.value)} />
                <Input label="Phone" value={truck.driver.phone} onChange={e => updateTruck(['driver','phone'], e.target.value)} />
              </div>
            </section>

            <section className="bg-white rounded-xl shadow p-4">
              <h2 className="font-semibold text-gray-900 mb-2">Tires and Sensors</h2>
              <p className="text-sm text-gray-500 mb-4">Read-only telemetry (live data from sensors). Editing is disabled.</p>
                ...
                {truck.tires.map((tire, idx) => (
                  <details key={tire.tireNo} className="group border rounded-lg">
                    ...
                    <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <Select label="Exception Types (exType)" value={tire.sensor.data.exType} disabled>
                      ...
                      <Input type="number" step="0.1" label="Pressure (kPa) tiprValue" value={tire.sensor.data.tiprValue} readOnly />
                      <Input type="number" step="0.1" label="Temperature (°C) tempValue" value={tire.sensor.data.tempValue} readOnly />
                      <Select label="Battery Level (bat) 0-4" value={tire.sensor.data.bat} disabled>
                        {[0,1,2,3,4].map(v => <option key={v} value={v}>{v}</option>)}
                      </Select>

                      <div className="col-span-1 md:col-span-2 mt-2">
                        <h3 className="text-sm font-semibold text-gray-800">Hub Temperature (hubdata)</h3>
                      </div>
                      <Input label="SN" value={tire.hub.sn} readOnly />
                      <Input label="SIM Number" value={tire.hub.data.simNumber} readOnly />
                      <Select label="Exception Types (exType)" value={tire.hub.data.exType} disabled>
                        <option value="">None</option>
                      ...
                      <Input type="number" step="0.1" label="Temperature (°C) tempValue" value={tire.hub.data.tempValue} readOnly />
          ...
          <div className="space-y-6">
            <section className="bg-white rounded-xl shadow p-4">
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Input type="number" step="0.000001" label="Longitude" value={truck.device.data.lng} readOnly />
                  <Input type="number" step="0.000001" label="Latitude" value={truck.device.data.lat} readOnly />
                </div>
                <div className="grid grid-cols-3 gap-4">
                  <Select label="Host Battery (bat1) 0-4" value={truck.device.data.bat1} disabled>{[0,1,2,3,4].map(v => <option key={v} value={v}>{v}</option>)}</Select>
                  <Select label="Repeater 1 (bat2) 0-4" value={truck.device.data.bat2} disabled>{[0,1,2,3,4].map(v => <option key={v} value={v}>{v}</option>)}</Select>
                  <Select label="Repeater 2 (bat3) 0-4" value={truck.device.data.bat3} disabled>{[0,1,2,3,4].map(v => <option key={v} value={v}>{v}</option>)}</Select>
                </div>
            ...
            <section className="bg-white rounded-xl shadow p-4">
              ...
              <button onClick={handleSave} className="w-full px-4 py-2 rounded-md bg-indigo-600 text-white text-sm hover:bg-indigo-700">Save Changes</button>
