...
export const getActiveAlertsByType = (alertType) => {
  return alertEvents.filter((alert) => alert.alert_type === alertType && !alert.acknowledged);
};
...
export const getDriversData = () => {
  return drivers.map((driver) => {
    ...
    const fleetGroup = assignedTruck
      ? fleetGroups.find((fg) => fg.id === assignedTruck.fleet_group_id)
      : null;
