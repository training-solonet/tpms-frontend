  ...
  async loadTruckLocationHistory(truckId, hours = 2) {
    try {
      ...
      try {
        ...
        const primaryUrl = this.makeUrl(
          `/api/location-history/${encodeURIComponent(truckId)}?${primaryParams.toString()}`
        );
        ...
        const legacyUrl = this.makeUrl(
          `/api/trucks/${encodeURIComponent(truckId)}/history?${fallbackParams.toString()}`
        );
