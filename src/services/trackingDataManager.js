...
export class TrackingDataManager {
  ...
  async getLocationHistory(truckId, truckNumber, hours = 2, maxPoints = 100) {
    ...
    try {
          ...
          if (list.length > 0) {
            const points = list
              .map((point) => ({
                ...
                timestamp: new Date(
                  point.recordedAt || point.timestamp || point.time || Date.now()
                ),
      ...
      try {
        const legacyUrl =
          `${this.apiConfig.BASE_URL}/api/trucks/${encodeURIComponent(truckId)}/history?` +
          new URLSearchParams({
  ...
  calculateRouteStatistics(points) {
    ...
    for (let i = 1; i < points.length; i++) {
      ...
      if (speed < 5) {
        idleTime += timeDiff;
        if (timeDiff > 5) {
          // Stop if idle for more than 5 minutes
          stops++;
        }
      } else {
        movingTime += timeDiff;
    ...
    const fuelUsed = points.length > 1 ? points[0].fuel - points[points.length - 1].fuel : 0;
  ...
  exportRouteData(vehicleId, points, format = 'json') {
    const summary = this.generateRouteSummary(
      vehicleId,
      points,
      this.calculateRouteStatistics(points)
    );
...
export class RealTimeTrackingBuffer {
  ...
  getStats() {
    ...
    return {
      ...
      totalPoints,
      averagePointsPerVehicle:
        this.buffers.size > 0 ? Math.round(totalPoints / this.buffers.size) : 0,
    };
  }
}
